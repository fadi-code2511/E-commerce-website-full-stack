...
const protect = async (req, res, next) => {
  ...
  if (
    ...
    req.headers.authorization.startsWith("Bearer") //If the request has an Authorization header and its value starts with the word "Bearer".
  ) {
    try {
...
const admin = (req, res, next) => {
  if (req.user && req.user.role === "admin") {
    ...
    res.status(403).json({ message: "not autharized as an admin" });
  }
};
