        ...
        <div className='w-full md:w-1/2 flex flex-col justify-center items-center p-8 md:p-12'>
            <form onSubmit={handleSubmit} action="" className='w-full max-w-md  p-8 rounded-lg border shadow-sm'>
                <div className='flex flex-col p-5 '>
                    ...
                    <p className='text-center mb-4 text-sm text-black'>Enter your name and email and password.</p>
                    <div className='flex flex-col gap-1'>
                        <label htmlFor="name" className='font-semibold'>Name</label>
                        <input value={name} onChange={(e)=>{setName(e.target.value)}}  type="text" required name='name'placeholder='Enter you name' className=' text-gray-700 p-1 placeholder:text-sm border rounded outline-blue-400 mb-2 ' />
                    </div>
                    <div className='flex flex-col gap-1'>
                    ...
                    <button className='bg-black p-2 text-sm rounded-md mb-4 text-white' >Sign up</button>
                    <p className=' text-center text-sm tracking-tighter'>Do you have an account? <Link to="/login" className='text-sm text-blue-400 font-bold  '>login</Link></p>
                </div>
            </form>
        </div>
        <div className=' hidden md:block w-1/2  '>
            <div className='w-full flex flex-col justify-center items-center '>
                <img src={registerImg} alt=""  className='h-[600px] object-cover w-full mb-1'/>
            </div>
        </div>
    </div>
  )
